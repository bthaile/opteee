...
- Multiple user interfaces: Web, Command Line, and Discord Bot
...
The OPTEEE system provides **three different user interfaces** to access the options trading knowledge base:

### **1. 🌐 Web Interface (Primary - Gradio)**

**The main web application built with Gradio - perfect for interactive browsing and detailed answers.**

**Files:** `app.py`, `templates/index.html`, `static/styles.css`, `static/script.js`

**Features:**
- ✅ **AI-powered Q&A** with OpenAI and Claude models
- ✅ **Semantic search** through 14,721+ transcript chunks
- ✅ **Source attribution** with direct video links and timestamps
- ✅ **Configurable settings** - number of results, AI provider, sorting options
- ✅ **Rich formatting** with markdown support
- ✅ **Example queries** built-in for quick testing
...
**Interface Elements:**
- **Search Input:** Natural language questions about options trading
- **Results Control:** Choose 1-10 results, select AI provider (OpenAI/Claude)
- **Sorting Options:** Sort by relevance, date, or combined
- **Formatted Output:** Markdown-formatted answers with clickable video links
- **Source Links:** Direct YouTube links with exact timestamps (`&t=XXX`)

### **2. 💻 Command Line Interface (Advanced)**

**Terminal-based interface for developers and power users.**

**Files:** `opteee_app.py`, `rag_pipeline.py`, `search_transcripts.py`

**Features:**
- ✅ **Interactive CLI** with configuration menu
- ✅ **Batch processing** for multiple queries
- ✅ **Configurable settings** - model selection, temperature, sources
- ✅ **Multiple providers** - OpenAI and Claude support
- ✅ **Formatted output** with text wrapping and source details
- ✅ **Session persistence** - maintains settings during session

**Usage Examples:**
```bash
# Interactive Q&A session
python3 opteee_app.py

# Single query with RAG
python3 rag_pipeline.py "What is gamma in options trading?"

# Vector search only
python3 search_transcripts.py "covered calls" --top-k 7
```

**CLI Commands:**
- `config` - Open configuration menu
- `exit`, `quit`, `q` - Exit application
- Any question - Get AI-powered answer with sources

**Configuration Options:**
- **Sources:** 1-20 results per query
- **Provider:** OpenAI or Claude
- **Models:** GPT-3.5/4, Claude variants
- **Temperature:** 0.0-1.0 for response creativity
- **Show Sources:** Toggle source attribution

### **3. 🤖 Discord Bot Interface (Community)**

**Discord bot for team collaboration and community access.**

**Files:** `discord/discord_bot.py`, `discord/requirements.txt`

**Features:**
- ✅ **Discord commands** for natural interaction
- ✅ **Message splitting** handles long responses automatically
- ✅ **Error handling** with user-friendly messages
- ✅ **Hugging Face integration** connects to hosted app
- ✅ **Activity status** shows bot availability
- ✅ **Logging system** tracks usage and errors
...
**Discord Commands:**
- `!search <query>` - Search options trading knowledge
- `!show_help` - Display help information

**Example Usage:**
```
!search What is gamma in options trading?
!search Explain covered call strategies
```

### **4. 📱 Additional UI Components**

**Supplementary interfaces for specialized tasks:**

#### **🔧 Download Tracker (HTML)**
**File:** `download_tracker.html` (generated by `generate_tracker_page.py`)

**Purpose:** Web-based interface for manual video download tracking
- ✅ **Progress tracking** with visual indicators
- ✅ **Direct download links** to y2mate and other services
- ✅ **Batch operations** for multiple videos
- ✅ **Filter and search** functionality
- ✅ **Export/import** progress data

#### **🗂️ Manual Processing Interface**
**File:** `manual_video_processor.py`

**Purpose:** Command-line interface for video processing management
- ✅ **Status scanning** shows processing state
- ✅ **Batch transcript processing** for audio files
- ✅ **Progress monitoring** with detailed statistics
- ✅ **File organization** separates processed from pending

### **🎯 UI Architecture & Integration**

#### **Common Backend Components:**
- **Vector Store:** FAISS index with 14,721+ chunks
- **RAG Pipeline:** `rag_pipeline.py` handles retrieval and generation
- **Search Engine:** `vector_search.py` for semantic similarity
- **Configuration:** `pipeline_config.py` for unified settings

#### **Data Flow:**
```
User Query → Interface (Web/CLI/Discord) → RAG Pipeline → Vector Search → LLM Generation → Formatted Response
```

#### **Shared Features Across All Interfaces:**
- 🔍 **Semantic Search** using sentence-transformers
- 🤖 **AI-powered Answers** with OpenAI/Claude models
- 📊 **Source Attribution** with video links and timestamps
- ⚙️ **Configurable Settings** for personalization
- 📈 **Consistent Results** from shared backend
...
- 🎨 You want rich formatting and visual elements
...
- 👥 You want team/community access
- 💬 You prefer chat-based interaction
- 📱 You want mobile notifications
- 🤝 You're collaborating with others

**Use Additional Tools when:**
- 🔧 You need to manage video processing pipeline
- 📊 You want detailed progress tracking
- 🗂️ You're organizing large amounts of data

### **🎨 UI Customization**

**Web Interface Styling:**
- **CSS:** `static/styles.css` - comprehensive styling
- **JavaScript:** `static/script.js` - interactive functionality
- **Templates:** `templates/index.html` - HTML structure
...
**Custom Themes:**
- Modify `static/styles.css` for color schemes
- Update `templates/index.html` for layout changes
- Customize `static/script.js` for behavior modifications

This comprehensive UI ecosystem ensures that users can access the options trading knowledge base through their preferred interface while maintaining consistent functionality and data quality across all platforms.
...
3. The system includes two ways to interact with the RAG pipeline:
   ...
   **b. Command-line Interface (Advanced):**
   ...
   - `--model "gpt-4"`: Use a specific OpenAI model (default is gpt-4o)
