...
# Native (no-Docker) launcher for the opteee Gradio/FastAPI service.
...
# We export DATABASE_URL here so the native host (127.0.0.1) wins over the .env
# value (host.docker.internal, which only resolves inside Docker).
export PORT=7860
...
# Refuse to start if the port is already held (e.g. the Docker container is still up)
if lsof -nP -iTCP:7860 -sTCP:LISTEN >/dev/null 2>&1; then
  echo "Port 7860 already in use (Docker container still running?). Exiting." >&2
  exit 1
fi
